...
u_tau_SSvar = u_tau_SS(p,n,freq,psi);
while abs(PI_SS)>10
    PI_SS=2.*PI_SS_var-log(1+PI_SS_var)-Karman.*p.V./...
    u_tau_SSvar+log(p.deltastar_SS(n)*p.V./p.kinvis)+0.51.*Karman+log(Karman);
    PI_SS_var=PI_SS_var+0.01;
end
    beta_c_SS = p.theta_SS(n)/p.tau_w_SS(n).*p.dP_dx_SS(n);
    Re_T_SS = u_tau_SSvar.*p.delta_SS(n).*sqrt(Cf_SSvar./2)./p.kinvis; %u_tau_SSvar, p.C_f./2 might be array
    phi_PP_SS =(p.tau_w_SS(n).^2.*p.deltastar_SS(n)./(p.V).*0.78.*...
    (1.8.*PI_SS_var.*beta_c_SS+6).*(freq*p.deltastar_SS(n)./p.V).^2)./...
