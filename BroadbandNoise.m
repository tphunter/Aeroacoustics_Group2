...
H=((1+1i)*exp(-4*1i*k_bar)*(1-theta^2))/(2*sqrt(pi)*(alpha-1)*k_x*sqrt(B));
...
I_TE_2=H*(exp(4*1i*k_bar)*(1-(1+1i)*fresnel(4*k_bar)))^epsilon+H*(exp(-2*1i*theta)+1i*(theta+k_bar_x+Avg_Mach*mu_bar-k_bar)*G); %Take a look at definition of epsilon (slide 8 lecture 4), it should not be an exponent.
...
I_LE_1=-1/pi*sqrt(2/(theta_minus*(k_bar_x+beta^2*k_bar)))*exp(-1i*theta)*fresnel(2*theta);
I_LE_2=exp(-1i*theta_bar)/(pi*theta*sqrt(2*pi*(k_bar_x+beta^2*k_bar)))*(1i*(1-exp(2*1i*theta_minus))-(1+1i)*(fresnel(4*k_bar)-exp(2*1i*theta_minus)*sqrt(2*k_bar/theta_plus)*fresnel(2*theta_plus)));
