...
%SS
alpha_SS(n)=p.V./p.U_c_SS(n); %p.U_c might be a vector (for sure)
...
I_TE_2_SS(n)=H_SS(n).*(exp(4.*1i.*k_bar(n)).*(1-(1+1i).*fresnel(4.*k_bar(n)))).^epsilon(n)+H_SS(n).*(exp(-2.*1i.*theta(n))+1i.*(theta(n)+k_bar_x(n)+p.Mach.*mu_bar(n)-k_bar(n)).*G_SS(n)); %Take a look at definition of epsilon (slide 8 lecture 4), it should not be an exponent.
...
l_y_TE_SS(n)=b.*p.U_c_SS(n)./p.omega; %p.U_c
...
%PS
alpha_PS(n)=p.V./p.U_c_PS(n); %p.U_c might be a vector (for sure)
...
phi_pp_omega_PS(n)=(p.tau_w_PS(n).^2.*p.deltastar_PS(n)./(p.V).*0.78.*...
    ...
    Re_T_PS(n).^(-0.57).*(p.omega.*p.deltastar_PS(n)./p.V)).^7);

l_y_TE_PS(n)=b.*p.U_c_PS(n)./p.omega; %p.U_c
