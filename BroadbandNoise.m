...
k_bar=param.omega*param.chord/(2*a_0);
epsilon=1/sqrt(1+1/(4*k_bar));
sigma=sqrt(param.x^2+(1-param.Mach^2)*(param.y^2+param.z^2));
k_y_bar=k_y*param.chord/2;
k_x_bar=k_x*param.chord/2;
Avg_Mach=param.V/param.a_0;
mu_bar=param.omega*param.chord/(2*param.a_0*beta^2); %This beta is not the angle I think 
B=k_bar_x-Avg_Mach*mu_bar+k_bar;
C=k_bar_x-mu_bar*(param.x/sigma-Avg_Mach);
...
Re_T=u_tau*param.delta_star*sqrt(C_f/2)/nu;
beta_c=theta/param.tau_w*param.dP_dx;
fun_PI=@(PI_var) 2*PI_var-log(1+PI_var)-kappa*param.V/u_tau-log(delta_star*param.V/nu)-0.51*kappa-log(kappa);
PI=fzero(fun_PI,1);
phi_pp_omega=(param.tau_w^2*param.delta_star/(param.V)*0.78*(1.8*PI*beta_c+6)*(param.omega*param.delta_star/param.V)^2)/(((param.omega*param.delta_star/param.V)^0.75+0.105)^3.7+(3.76*Re_T^(-0.57)*(param.omega*param.delta_star/param.V))^7);
l_y_TE=b*U_c/param.omega;
phi_pp=phi_pp_omega*l_y_TE/pi;

%3.- Power spectral density
S_pp_TE=(k_bar*param.z/(2*pi*sigma^2))^2*2*param.chord*integral(phi_pp*sin(param_L/param_chord*(k_bar_y-k_bar*param.z/sigma))^2/((k_bar_y-k_bar*param.z/sigma)^2)*abs(I_TE)^2,-inf,inf);
...
theta_bar=k_bar_x*(1-Avg_Mach*param.x/sigma)/(beta^2)-pi/4;
...
phi_omegaomega_omega=u_bar_prime^2*L_t/(2*pi*param.V)*(1+8/3*(k_x+k_e)^2)/((1+(k_x/k_e)^2)^(211/6))
l_y_LE=(tau(1/3)/tau(5/6))^2*(k_x/k_e)^2/((3+8*(k_x+k_e)^2)*sqrt(1+(k_x+k_e)^2));

phi_omegaomega=param.V*phi_omegaomega_omega*l_y_LE/pi;

%3.- Power spectral density
S_pp_LE=(param.rho_0*k_bar*param.z/(sigma^2))^2*pi*param.V*param.L/2*integral(phi_omegaomega*sin(param.L/2*(k*param.y/sigma-k*param.y))^2/(pi/2*(k*param.y/sigma-k*param.y)^2*abs(I_LE)^2),-inf,inf);